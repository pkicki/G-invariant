...
class GroupInvariance(tf.keras.Model):
    def __init__(self, num_features, activation=tf.keras.activations.tanh):
        ...
        self.features = [
            ...
            tf.keras.layers.Dense(6 * 2, None),
        ]
...
class SegmentNet(tf.keras.Model):
    def __init__(self, num_features):
        ...
        self.fc = [
            tf.keras.layers.Dense(24, activation),
            tf.keras.layers.Dense(1),
        ]
...
class Maron(tf.keras.Model):
    def __init__(self, num_features, activation=tf.keras.activations.tanh):
        ...
        self.a = list(set([p for x in partitionfunc(6, 4, l=0) for p in permutations(x)]))
        self.a = [(x[:2] + (0, 0, 0, 0) + x[2:] + (0, 0, 0, 0)) for x in self.a]
