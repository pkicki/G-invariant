...
def groupAvereaging(inputs, operation):
    ...
    x = tf.reduce_mean(tf.stack([x1, x2, x3], -1), -1)
    return x
...
class GroupInvariance(tf.keras.Model):
    def __init__(self, num_features, activation=tf.keras.activations.tanh):
        ...
        self.features = [
            ...
            tf.keras.layers.Dense(3 * 64, tf.keras.activations.tanh),
        ]
    ...
    def call(self, inputs, training=None):
        ...
        for layer in self.features:
            x = layer(x)
        x = tf.reshape(x, (bs, n_points, -1, 3))
        a, b, c = tf.unstack(x, axis=1)
        # x = a[:, :, 0] + b[:, :, 1] + c[:, :, 2] + d[:, :, 3] \
        #    + b[:, :, 0] + c[:, :, 1] + d[:, :, 2] + a[:, :, 3] \
        #    + c[:, :, 0] + d[:, :, 1] + a[:, :, 2] + b[:, :, 3] \
        #    + d[:, :, 0] + a[:, :, 1] + b[:, :, 2] + c[:, :, 3]
...
class GroupInvarianceConv(tf.keras.Model):
    def __init__(self, num_features, activation=tf.keras.activations.tanh):
        ...
        self.features = [
            ...
            tf.keras.layers.Conv1D(3 * self.last_n, 1, padding='same'),
        ]
        self.fc = [
    ...
    def call(self, inputs, training=None):
        ...
        # x =
        a, b, c = tf.unstack(x, axis=1)
        a = tf.reshape(a, (-1, 3, self.last_n))
        b = tf.reshape(b, (-1, 3, self.last_n))
        c = tf.reshape(c, (-1, 3, self.last_n))
...
class SegmentNet(tf.keras.Model):
    ...
    def process(self, quad):
        ...
        s3 = tf.concat([quad[:, 2], quad[:, 2]], axis=-1)
