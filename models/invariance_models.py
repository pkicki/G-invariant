...
class PointProcessor(tf.keras.Model):
...
class InsideNet(tf.keras.Model):
    def __init__(self):
        ...
        self.point_processor = PointProcessor(n)
    ...
    def call(self, quad, point, training=None):
        quad_ft = self.quadrangle_processor(quad)
        point_ft = self.point_processor(point)

        x = tf.concat([quad_ft, point_ft], -1)
