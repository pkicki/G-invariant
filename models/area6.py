...
def groupAvereaging(inputs, operation):
    ...
    x = tf.reduce_mean(tf.stack([x1, x2, x3, x4, x5, x6], -1), -1)
    return x
...
class GroupInvariance(tf.keras.Model):
    def __init__(self, num_features, activation=tf.keras.activations.tanh):
        ...
        self.features = [
            ...
            tf.keras.layers.Dense(6 * 64, None),
        ]
    ...
    def call(self, inputs, training=None):
        ...
        for layer in self.features:
            x = layer(x)
        x = tf.reshape(x, (bs, n_points, -1, 6))
        a, b, c, d, e, f = tf.unstack(x, axis=1)
        x = a[:, :, 0] * b[:, :, 1] * c[:, :, 2] * d[:, :, 3] * e[:, :, 4] * f[:, :, 5] \
            + b[:, :, 0] * c[:, :, 1] * d[:, :, 2] * e[:, :, 3] * f[:, :, 4] * a[:, :, 5]\
            + c[:, :, 0] * d[:, :, 1] * e[:, :, 2] * f[:, :, 3] * a[:, :, 4] * b[:, :, 5] \
            + d[:, :, 0] * e[:, :, 1] * f[:, :, 2] * a[:, :, 3] * b[:, :, 4] * c[:, :, 5]\
            + e[:, :, 0] * f[:, :, 1] * a[:, :, 2] * b[:, :, 3] * c[:, :, 4] * d[:, :, 5]\
...
class GroupInvarianceConv(tf.keras.Model):
    def __init__(self, num_features, activation=tf.keras.activations.tanh):
        ...
        self.features = [
            ...
            tf.keras.layers.Conv1D(6 * self.last_n, 1, padding='same'),
        ]
        self.fc = [
    ...
    def call(self, inputs, training=None):
        ...
        x = tf.reshape(x, (-1, 6, 6, self.last_n))
        a, b, c, d, e, f = tf.unstack(x, axis=1)
...
class SimpleNet(tf.keras.Model):
    def __init__(self, num_features):
        ...
        activation = tf.keras.activations.tanh
        self.features = [
            tf.keras.layers.Dense(108, activation),
            #tf.keras.layers.Dense(64, activation),
...
class SegmentNet(tf.keras.Model):
    def __init__(self, num_features):
        ...
        self.features = [
            tf.keras.layers.Dense(64, activation),
            tf.keras.layers.Dense(84, activation),
            tf.keras.layers.Dense(6 * num_features, activation),
        ]
    ...
    def process(self, quad):
        ...
        s6 = tf.concat([quad[:, 5], quad[:, 0]], axis=-1)
...
class Maron(tf.keras.Model):
    def __init__(self, num_features, activation=tf.keras.activations.tanh):
        ...
        self.features = [
            tf.keras.layers.Dense(13, activation),
            #tf.keras.layers.Dense(64, activation),
            # tf.keras.layers.Dense(2048, activation),
            # tf.keras.layers.Dense(6 * num_features, activation),
            tf.keras.layers.Dense(num_features, activation),
        ...
        #self.a = list(set([p for x in partitionfunc(2, 11, l=0) for p in permutations(x)]))
        self.a = []
        for x in partitionfunc(2, 12, l=0):
    ...
    def call(self, x, training=None):
        def inv(a, b, c, d, e, f, g, h, i, j, k, l):
            p = self.f
            x1 = a ** p[:, 0]
            x2 = b ** p[:, 1]
            x3 = c ** p[:, 2]
            ...
            x11 = k ** p[:, 10]
            x12 = l ** p[:, 11]
            mul = x1 * x2 * x3 * x4 * x5 * x6 * x7 * x8 * x9 * x10 * x11 * x12
            mulnn = self.mulnn(tf.stack([x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12], axis=-1))[:, :, 0]
        ...
        x = tf.reshape(x, (-1, 12))
        a, b, c, d, e, f, g, h, i, j, k, l = tf.unstack(x[:, :, tf.newaxis], axis=1)

        def term():
            p1, l1 = inv(a, b, c, d, e, f, g, h, i, j, k, l)
            p2, l2 = inv(f, a, b, c, d, e, l, g, h, i, j, k)
            p3, l3 = inv(e, f, a, b, c, d, k, l, g, h, i, j)
            p4, l4 = inv(d, e, f, a, b, c, j, k, l, g, h, i)
            p5, l5 = inv(c, d, e, f, a, b, i, j, k, l, g, h)
            p6, l6 = inv(b, c, d, e, f, a, h, i, j, k, l, g)
...
class MessagePassing(tf.keras.Model):
    ...
    def call(self, inputs, training=None):
        ...
        for i in range(1):
            ...
            Mfa = self.process(tf.concat([Uf, Ua], axis=1), self.M)

            Ua = self.process(tf.concat([Mfa, Ua, a], axis=1), self.U)
        ...
        x = self.process(tf.concat([Ua, Ub, Uc, Ud, Ue, Uf], axis=1), self.R)
