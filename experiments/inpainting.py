...
def main(args):
    # 1. Get datasets
    train_ds, train_size = scenarios.inpainting_dataset(args.scenario_path)
    val_ds, val_size = scenarios.inpainting_dataset(args.scenario_path.replace("train", "val"))
    ...
    for epoch in range(args.num_epochs):
        ...
        acc = []
        for i, quad, img in _ds('Train', dataset_epoch, train_size, epoch, args.batch_size):
            # 5.1.1. Make inference of the model, calculate losses and record gradients
            with tf.GradientTape(persistent=True) as tape:
                output = model(quad)
                model_loss = tf.keras.losses.binary_crossentropy(img, output)
            ...
            with tfc.summary.record_summaries_every_n_global_steps(args.log_interval, train_step):
                tfc.summary.image('metrics/gt', img[:, :, :, tf.newaxis], step=train_step)
                tfc.summary.image('metrics/pred', output[:, :, :, tf.newaxis], step=train_step)
                tfc.summary.image('metrics/pred_binary', tf.cast(output[:, :, :, tf.newaxis] > 0.5, tf.float32), step=train_step)
        ...
        acc = []
        for i, quad, img in _ds('Validation', val_ds, val_size, epoch, args.batch_size):
            ...
            with tfc.summary.record_summaries_every_n_global_steps(args.log_interval, val_step):
                ...
                tfc.summary.image('metrics/gt', img[:, :, :, tf.newaxis], step=val_step)
                tfc.summary.image('metrics/pred', output[:, :, :, tf.newaxis], step=val_step)
                tfc.summary.image('metrics/pred_binary', tf.cast(output[:, :, :, tf.newaxis] > 0.5, tf.float32), step=val_step)
