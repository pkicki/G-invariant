...
import sys
import numpy as np
from utils.permutation_groups import Human
...
def main(args):
    # 1. Get datasets
    train_ds, train_size, val_ds, val_size, test_ds, test_size, n_cls = scenarios.action_recognition_dataset(args.scenario_path)
    ...
    if args.model == "FC_G-inv":
        model = GroupInvariance(Human, args.n, n_cls)
    ...
    for epoch in range(args.num_epochs):
        ...
        acc = []
        for i, pose, cls, in _ds('Train', dataset_epoch, train_size, epoch, args.batch_size):
            ...
            with tf.GradientTape(persistent=True) as tape:
                ...
                oh = tf.one_hot(cls, n_cls, axis=-1)
            ...
            acc = acc + list(tf.cast(tf.equal(idx, cls), tf.float32).numpy())
        ...
        acc = []
        for i, pose, cls in _ds('Validation', val_ds, val_size, epoch, val_bs):
            ...
            idx = tf.argmax(pred, axis=-1)
            acc = acc + list(tf.cast(tf.equal(idx, cls), tf.float32))
