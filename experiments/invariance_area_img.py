...
from models.area7_small import ConvImg
...
def main(args):
    ...
    val_ds, val_size = scenarios.area_img_dataset(args.scenario_path.replace("train", "val"))
    ...
    for epoch in range(args.num_epochs):
        ...
        acc = []
        for i, quad, area, img in _ds('Train', dataset_epoch, train_size, epoch, args.batch_size):
            # 5.1.1. Make inference of the model, calculate losses and record gradients
            with tf.GradientTape(persistent=True) as tape:
                #quad = proj(quad)
                #quad = tf.reshape(quad, (-1, 4, 2))
        ...
        acc = []
        for i, quad, area, img in _ds('Validation', val_ds, val_size, epoch, val_bs):
