...
from models.ae2 import Decoder, GroupInvariance, SimpleNet, Conv1d, SegmentNet, \
    GroupInvarianceConv
...
def main(args):
    ...
    for epoch in range(args.num_epochs):
        ...
        acc = []
        for i, quad1, quad2 in _ds('Train', dataset_epoch, train_size, epoch, args.batch_size):
            # 5.1.1. Make inference of the model, calculate losses and record gradients
            with tf.GradientTape(persistent=True) as tape:
                features = model(quad1, quad2, training=True)
                ...
                output = tf.reshape(output, (-1, 4, 2, 2))
                output1 = output[:, :, :, 0]
                output2 = output[:, :, :, 1]
                o1 = tf.roll(output1, 1, 1)
                o2 = tf.roll(output1, 1, 1)
                o3 = tf.roll(output1, 2, 1)
                o4 = tf.roll(output1, 3, 1)
                ...
                model_loss1 = tf.reduce_min(tf.stack([l1, l2, l3, l4], -1), -1)

                o1 = tf.roll(output2, 1, 1)
                o2 = tf.roll(output2, 1, 1)
                o3 = tf.roll(output2, 2, 1)
                o4 = tf.roll(output2, 3, 1)
                ...
                model_loss2 = tf.reduce_min(tf.stack([l1, l2, l3, l4], -1), -1)
