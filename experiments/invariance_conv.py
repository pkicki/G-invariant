...
def main(args):
    # 1. Get datasets
    train_ds, train_size = scenarios.convexity_dataset(args.scenario_path)
    val_ds, val_size = scenarios.convexity_dataset(args.scenario_path.replace("train", "val"))
    ...
    for epoch in range(args.num_epochs):
        ...
        for i, quad, convexity, in _ds('Train', dataset_epoch, train_size, epoch, args.batch_size):
            # 5.1.1. Make inference of the model, calculate losses and record gradients
            with tf.GradientTape(persistent=True) as tape:
                pred = model(quad, training=True)[:, 0]
                ...
                model_loss = tf.keras.losses.binary_crossentropy(convexity, pred)
            ...
            accuracy(convexity > 0.5, pred > 0.5)
        ...
        acc = []
        for i, quad, convexity in _ds('Validation', val_ds, val_size, epoch, val_bs):
